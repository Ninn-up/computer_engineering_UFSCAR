...
class HashTable:
    ...
    def fn_aux(self, key):  # Utiliza-se o método da multiplicação para calcular o índice
        index = floor(self.size * (key * 0.618 % 1))  # Para a constante, usa-se o valor 0,618 (Knuth)
        return index

    # Função Hash para Linear Probing
    def fn_hash(self, key, i):
        index = (self.fn_aux(key) + i) % self.size  # O index depende da fn. aux e do i
        return index
