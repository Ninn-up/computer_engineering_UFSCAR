...
//     for(i=0; i < ha->TABLE_SIZE; i++){
//         newPos = sondagemLinear(pos,i,ha->TABLE_SIZE);  // Fórmula para calcular o novo índice
...
int insereHash_EncadeamentoSeparado(Hash* ha, struct aluno al){
    ...
    for(i=0; i < ha->TABLE_SIZE; i++){
        ...
        if(ha->itens[newPos] == NULL){
            ...
            if (insere_lista_final(novaLista, al)) {
                ha->itens[newPos] = novaLista;
                ha->qtd++;
                return 1;
            }
            ...
            if (insere_lista_final(ha->itens[newPos], al)) {
                ha->qtd++;
                return 1;
            }
        }
...
int buscaHash_EncadeamentoSeparado(Hash* ha, int mat, struct aluno* al){
    ...
    for(i=0; i < ha->TABLE_SIZE; i++){
        ...
        if(ha->itens[newPos] == NULL)
            ...
            if (consulta_lista_mat(ha->itens[newPos], mat, al)) {
                return 1;
            }
...
Lista *get_lista(Hash* ha, int pos) {
    if (ha == NULL || pos < 0 || pos >= ha->TABLE_SIZE) {
        return NULL;
    }
    return ha->itens[pos];
}